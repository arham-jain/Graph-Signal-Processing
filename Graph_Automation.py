    ...
    def graph_signal(self):
        ...
        for i in range(len(distance)):
            if distance[i] <= threshold:
                filtered_node1.append(ref_nodes[node1[i]])
                filtered_node2.append(ref_nodes[node2[i]])
        ...
        for k,v in self.kpi_dict.items():
            signal[ref_nodes[k]] = v
        self.signal = np.asarray(signal)

    def gsp_plots(self):
        self.G.set_coordinates('spring')
        plotting.plot_graph(self.G)

        fig, axes = plt.subplots(1, 2, figsize=(10, 4))
        self.G.plot_signal(gsp.signal, ax=axes[0])
        ...
        self.G.plot_signal(self.G.U[:, 7], vertex_size=50, ax=axes[1])
        plt.show()
